...
export default function RestaurantImageGallery({ images, restaurantName }: RestaurantImageGalleryProps) {
  ...
  const normalizedImages = (images || []).map((img: any) => {
    ...
    // Handle JSONB objects from Supabase (already parsed)
    if (typeof img === 'object' && img.url) return { url: img.url, description: img.description || '' };
    // Fallback: try to extract url from object
    if (typeof img === 'object') return { url: img.url || String(img), description: img.description || '' };
    ...
    <>
      ...
      <div className="w-full min-w-0 max-w-full">
        ...
        <div className="block md:hidden">
          <div className="space-y-2 w-full min-w-0">
            ...
            >
              ...
              {/* View all indicator / Click hint */}
              {normalizedImages.length > 1 && (
                <div className="absolute top-4 right-4 rounded-full bg-white/90 px-3 py-1.5 backdrop-blur-sm">
                  <span className="text-sm font-semibold text-gray-900">View All {normalizedImages.length}</span>
                </div>
              )}
              {/* Click indicator when there's only one image */}
              {normalizedImages.length === 1 && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/10 transition-all duration-300">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full">
                    <span className="text-sm font-semibold text-gray-900">Click to view</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="hidden md:block">
          <div className="grid h-[60vh] grid-cols-4 grid-rows-2 gap-3 overflow-hidden rounded-2xl w-full min-w-0">
            ...
            >
              ...
              {normalizedImages.length > 1 && (
                <div className="absolute top-6 right-6 rounded-full bg-white/95 px-4 py-2 backdrop-blur-sm shadow-lg">
                  <span className="text-sm font-semibold text-gray-900">View All {normalizedImages.length} photos</span>
                </div>
              )}
              {/* Click indicator when there's only one image */}
              {normalizedImages.length === 1 && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/10 transition-all duration-300">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/90 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg">
                    <span className="text-base font-semibold text-gray-900">Click to view photo</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {displayImages.length > 1 && displayImages.slice(1, 5).map((image, index) => (
              <div
                key={index}
        ...
        {selectedImage !== null && (
          ...
          >
            ...
            <div className="relative flex items-center justify-center max-h-[90vh] max-w-[90vw]">
              ...
              >
                <span className="text-white font-medium">{selectedImage + 1} / {normalizedImages.length}</span>
              </motion.div>
            </div>
          </motion.div>
        )}
